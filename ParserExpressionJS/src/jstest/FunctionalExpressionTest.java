...
public class FunctionalExpressionTest extends BaseJavascriptTest<Engine> {
    ...
    protected static <T extends BaseTest> void test(final Class<T> type, final BiFunction<Language, Boolean, T> cons,
                                                    final String[] args, final AbstractTests tests) {
        test(type, cons, tests, args, ARITHMETIC_FUNCTIONS);
    }

    protected static <T extends BaseTest> void test(final Class<T> type, final BiFunction<Language, Boolean, T> cons,
                                                final AbstractTests tests, final String[] args, final Dialect parsed) {
        cons.apply(
